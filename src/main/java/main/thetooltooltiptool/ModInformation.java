package main.thetooltooltiptool;

/*
 * Basic information your mod depends on.
 */

public class ModInformation
{

   public static final String NAME = "The Tool Tooltip Tool";
   public static final String ID = "thetooltooltiptool";
   public static final String CHANNEL = "TheToolTooltipTool";
   public static final String DEPEND = "";
   public static final String VERSION = "0.3.0";
   public static final String CLIENTPROXY = "main.thetooltooltiptool.proxies.ClientProxy";
   public static final String COMMONPROXY = "main.thetooltooltiptool.proxies.CommonProxy";
}
